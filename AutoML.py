...
#Datatypes
    def change_column_dtype(df):
        ...
        dtype = st.selectbox("Type", ["object", "int64", "float64", "bool", "datetime64"])
    ...
    def dataviz(d):
        ...
        if plot_type == "Histogram / Distribution":
            ...
            cat_cols = d.select_dtypes(include='object').columns
            num_cols = d.select_dtypes(include=['int64', 'float64']).columns
            x = st.selectbox("X-axis (categorical)", cat_cols, key="bar_x")
            y = st.selectbox("Y-axis (numeric)", num_cols, key="bar_y")
            fig, ax = plt.subplots()
            sns.barplot(data=d, x=x, y=y, ax=ax)
            st.pyplot(fig)

        elif plot_type == "Violinplot":
            col1 = st.selectbox("Y-axis (numeric)", num_cols, key='violin_y')
            col2 = st.selectbox("X-axis (categorical)", d.select_dtypes('object').columns, key='violin_x')
            fig, ax = plt.subplots()
            sns.violinplot(data=d, x=col2, y=col1, ax=ax)
            st.pyplot(fig)
...
#Datatypes
    if st.sidebar.checkbox("Change Dtypes"):
        df=change_column_dtype(df)


    #labelEncoding
    if st.sidebar.checkbox('Label Encode'):                         
        df=labelencoding(df)

    #RENAME
    if st.sidebar.checkbox("rename"):
        df=rename(df)
    ...
    if st.sidebar.checkbox("📈 Univariate Plot"):
    ...
    if st.sidebar.checkbox("⚖️ Balance Dataset"):
...
if uploaded_file:
    if uploaded_file.type == "text/csv":
        ...
        if len(df.sheet_names) > 1:
            x=st.radio('select sheet',df.sheet_names)
            df = pd.read_excel(df, sheet_name=x)
        else: 
    ...
    if target:
        ...
        if task == "Classification":
            ...
            if result is not None:
                data, best_model = result
                
            else:
                data, best_model = None, None
        else:
            data, best_model = regression(x, y)
